...
@section('heading', 'التحقق الثنائي')
@section('subheading', 'أدخل الرمز المُرسَل إلى هاتفك لإتمام تسجيل الدخول')

@section('content')
    <form id="verify-form" method="POST" action="{{ route('login.2fa.verify') }}" class="space-y-4" novalidate>
        @csrf

        <div class="bg-white/[0.04] border border-white/[0.08] rounded-xl p-3.5 flex items-center gap-3">
            <div class="w-10 h-10 rounded-lg flex items-center justify-center bg-amber-500/15 text-amber-400 shrink-0">
            ...
            <div class="min-w-0 text-sm">
                ...
                <p class="text-white/50 text-xs font-mono" dir="ltr">+{{ $phone_mask }}</p>
            </div>
        </div>

        <div>
            <label class="block text-[12px] font-semibold text-white/60 mb-2">رمز التحقق</label>
            <input name="code" type="text" inputmode="numeric" maxlength="6" autocomplete="one-time-code" required autofocus dir="ltr"
            ...
            <p class="mt-3 text-[11px] text-white/40 text-center">
                لم يصلك الرمز؟
                <button type="button" id="resend-btn" class="text-amber-400 hover:underline font-semibold bg-transparent border-0 cursor-pointer">أعد الإرسال</button>
            </p>
        </div>

        <button type="submit" id="submit-btn" class="btn-accent has-ripple w-full text-white rounded-xl py-3.5 font-bold text-sm flex items-center justify-center gap-2 mt-4">
            <span id="btn-spinner" class="hidden w-[18px] h-[18px] rounded-full spinner animate-spin"></span>
            <span id="btn-text">تحقّق ودخول</span>
        </button>
    </form>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            $form.on('submit', function (e) {
                ...
                $.ajax({ url: $form.attr('action'), type: 'POST', data: $form.serialize(), dataType: 'json', headers: { 'Accept': 'application/json' } })
                    .done(r => { toastr.success(r.message); setTimeout(() => window.location.href = r.redirect, 400); })
                    .fail(xhr => {
                        $('#submit-btn').prop('disabled', false);
                        $('#btn-text').text('تحقّق ودخول');
                        ...
                        } else if (xhr.responseJSON?.redirect) {
            ...
            $('#resend-btn').on('click', function () {
                $.post('{{ route("login.2fa.resend") }}', {})
                    .done(r => toastr.success(r.message))
                    .fail(xhr => toastr.error(xhr.responseJSON?.message ?? 'فشل الإرسال'));
            });
        });
    </script>
